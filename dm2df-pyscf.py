...
for iconf in conf_list:
    ...
    naux = auxmol.nao_nr()  # 208
    # 2-centers 2-electrons integral
    eri2c = auxmol.intor('int2c2e_sph') # dim=(208, 208)
    # 3-centers 2-electrons integral
    eri3c = pmol.intor('int3c2e_sph', shls_slice=(0,mol.nbas,0,mol.nbas,mol.nbas,mol.nbas+auxmol.nbas)) #dim(115, 115, 208) 
    eri3c = eri3c.reshape(mol.nao_nr(), mol.nao_nr(), -1) ## seems redundant.  
    # Load 1-electron reduced density-matrix
    dm=np.load(inp.path2qm+"density_matrices/dm_conf"+str(iconf+1)+".npy") #dim=(115,115)
